function flag = LoadEFOptimumAndRefine1()
...
vis_fps = 240;
nfe_vis = max(2, round(params.ef.tf * vis_fps));

t_enriched = linspace(0, params.ef.tf, length(x_enriched));
t_vis      = linspace(0, params.ef.tf, nfe_vis);

params.ef.enriched_x     = interp1(t_enriched, x_enriched,     t_vis, 'linear').';
params.ef.enriched_y     = interp1(t_enriched, y_enriched,     t_vis, 'linear').';
params.ef.enriched_theta = interp1(t_enriched, theta_enriched, t_vis, 'linear').';

params.ef.enriched_v   = interp1(t_enriched, v_enriched,   t_vis, 'linear').';
params.ef.enriched_a   = interp1(t_enriched, a_enriched,   t_vis, 'linear').';
params.ef.enriched_phy = interp1(t_enriched, phy_enriched, t_vis, 'linear').';
params.ef.enriched_w   = interp1(t_enriched, w_enriched,   t_vis, 'linear').';

params.ef.enriched_up    = interp1(t_enriched, up_enriched,    t_vis, 'previous').';
params.ef.enriched_down  = interp1(t_enriched, down_enriched,  t_vis, 'previous').';
params.ef.enriched_left  = interp1(t_enriched, left_enriched,  t_vis, 'previous').';
params.ef.enriched_right = interp1(t_enriched, right_enriched, t_vis, 'previous').';
